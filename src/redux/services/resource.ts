...
import {FileMapType, ResourceContentMap, ResourceMapType, ResourceMetaMap} from '@shared/models/appState';
import {ClusterAccess} from '@shared/models/config';
import {K8sResource} from '@shared/models/k8sResource';
import {FileLocation, isFileLocation, isPreviewResourceLocation} from '@shared/models/objectLocation';
...
export function isFileResource(resource: K8sResource): resource is K8sResource & {sourceLocation: FileLocation} {
  return isFileLocation(resource.sourceLocation) && !resource.isUnsaved;
}
...
export function isPreviewResource(resource: K8sResource) {
  return isPreviewResourceLocation(resource.sourceLocation);
}
...
export function saveResource(resource: K8sResource, newValue: string, fileMap: FileMapType) {
  ...
  if (isFileResource(resource)) {
    const fileEntry = fileMap[resource.sourceLocation.target];
...
export function removeResourceFromFile(
  ...
  if (!isFileLocation(removedResource.sourceLocation)) {
    throw new Error(`[removeResourceFromFile]: Specified resource is not from a file.`);
  }
  const fileEntry = fileMap[removedResource.sourceLocation.target];
  if (!fileEntry) {
    throw new Error(`Failed to find fileEntry for resource with path ${removedResource.sourceLocation.target}`);
  }
  ...
  const resourceIds = getResourcesForPath(removedResource.sourceLocation.target, resourceMap)
    .sort((a, b) => {
