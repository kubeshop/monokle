...
>('main/saveUnsavedResource', async ({resourceId, absolutePath, providedResource}, thunkAPI) => {
  const mainState = thunkAPI.getState().main;
  const resource = providedResource || mainState.resourceMap[resourceId];
